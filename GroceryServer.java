...
public class GroceryServer {
    ...
    static void serveFile(HttpExchange e, String file, String type) throws IOException {
        ...
        if (!f.exists()) { send(e, 404, "File not found"); return; }
    ...
    static void register(HttpExchange e) throws IOException {
        ...
        if (u.length() < 3 || pw.length() < 4) {
            sendJson(e, "{\"success\":false,\"message\":\"Username must be at least 3 characters and password at least 4 characters.\"}");
            return;
        }
    ...
    static void login(HttpExchange e) throws IOException {
        ...
        try (PreparedStatement s = con.prepareStatement(
            ...
            s.setString(1,u); s.setString(2,pw);
            ResultSet r=s.executeQuery(); ok=r.next();
        } catch(Exception ex) {}
    ...
    static void products(HttpExchange e) throws IOException {
        ...
        } catch(Exception ex) { b.append("{\"error\":\"database error\"}"); }
        b.append("]");
    ...
    static void stack(HttpExchange e) throws IOException {
        ...
        try(PreparedStatement s=con.prepareStatement(
            ...
            s.setInt(1,id); ResultSet r=s.executeQuery(); boolean first=true;
            while(r.next()){
                if(!first)b.append(",");
                first=false;
                b.append("{\"id\":").append(r.getInt("id"))
                 ...
                 .append(",\"created\":\"").append(json(String.valueOf(r.getTimestamp("created_at")))).append("\"}");
            }
        }catch(Exception ex){}
        b.append("]"); sendJson(e,b.toString());
    }
    ...
    static void push(HttpExchange e) throws IOException {
        ...
        try(PreparedStatement s=con.prepareStatement(
            ...
            s.setInt(1,product); s.setString(2,box); s.setInt(3,qty); s.executeUpdate();
            sendJson(e,"{\"success\":true}");
        }catch(Exception ex){sendJson(e,"{\"success\":false,\"message\":\"Could not add stock.\"}");}
    }
    ...
    static void pop(HttpExchange e) throws IOException {
        ...
        try(PreparedStatement s=con.prepareStatement(
            ...
            s.setInt(1,product); ResultSet r=s.executeQuery();
            if(!r.next()){sendJson(e,"{\"success\":false,\"message\":\"Stack is empty.\"}");return;}
            ...
            if(qty>=current){
                ...
                try(PreparedStatement u=con.prepareStatement("UPDATE stock SET quantity=quantity-? WHERE id=?")){
                    u.setInt(1,qty);u.setInt(2,id);u.executeUpdate();
                }
            }
            sendJson(e,"{\"success\":true,\"message\":\"Top stock removed using LIFO.\"}");
        }catch(Exception ex){sendJson(e,"{\"success\":false,\"message\":\"Could not remove stock.\"}");}
    }
    ...
    static void send(HttpExchange e,int code,String s)throws IOException{
        ...
        e.sendResponseHeaders(code,d.length);e.getResponseBody().write(d);e.close();
    }
}
